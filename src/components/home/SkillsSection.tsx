...
const groups: CapabilityGroup[] = [
  {
    ...
    description: 'Flutter-first. Cross-platform apps with Firebase, real-time data, and production-grade state management.',
    skillIds: ['flutter', 'firebase', 'androidstudio'],
  },
  {
    title: 'Web & full-stack',
    description: 'React on the front, Node/Express on the back. Typed, tested, and shipped to small teams of real users.',
    skillIds: ['react', 'javascript', 'node', 'expressjs'],
  },
  {
    title: 'AI & data',
    description: 'Practical AI: integrating OpenAI/Gemini, prototyping vision models, and running data through Python pipelines.',
    skillIds: ['python', 'mongodb', 'mysql'],
  },
  {
    title: 'Foundations & tooling',
    description: 'Git, CI workflows, design tools, and the CS fundamentals that keep code maintainable.',
      ...
      <div className="container mx-auto px-4 max-w-[1200px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-12 lg:gap-y-16">
          {groups.map((group, idx) => (
            ...
            >
              <p className="eyebrow eyebrow-accent mb-3">0{idx + 1}</p>
              ...
              <div className="flex flex-wrap items-center gap-3">
                {group.skillIds.map((id) => (
                  <div
                    key={id}
                    className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/[0.08] bg-white/[0.02] text-xs text-muted-foreground"
                  >
                    <img
                      ...
                      onError={(e) => {
                        (e.target as HTMLImageElement).src = FALLBACK_SKILL_ICON;
                      }}
                    />
        ...
        <div className="mt-16">
          ...
          >
            See the full technology list <ArrowRight className="h-4 w-4" />
          </Link>
        </div>
      </div>
    </section>
